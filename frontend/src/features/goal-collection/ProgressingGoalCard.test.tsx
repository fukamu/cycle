...
describe("ProgressingGoalCard", () => {
  ...
  it("does not expose a saved review schedule on the Home card", () => {
    const reviewDate = "2026-09-25";
    const scheduledGoal: ActiveGoal = {
      ...activeGoal,
      currentWork: {
        ...activeGoal.currentWork,
        reviewSchedule: { reviewDate, reviewScheduleRevision: 2 },
      },
    };

    renderCard(scheduledGoal);

    const card = screen.getByRole("article", {
      name: scheduledGoal.currentVersion.body,
    });
    expect(within(card).queryByText(reviewDate)).not.toBeInTheDocument();
