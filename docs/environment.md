...
| GitHub variable | `AI_MONTHLY_BUDGET_USD`             | Cloud Run runtime                 |
| GitHub variable | `AI_MAX_GENERATIONS_PER_USER_24H`   | Cloud Run runtime                 |
| GitHub variable | `RECAPTCHA_SCORE_THRESHOLD`         | Cloud Run runtime                 |
| GitHub variable | `RATE_ANONYMOUS_CREATE_PER_IP_HOUR` | Cloud Run runtime                 |
| GitHub variable | `RATE_ANONYMOUS_CREATE_PER_IP_24H`  | Cloud Run runtime                 |
| GitHub variable | `RATE_AI_PER_USER_MINUTE`           | Cloud Run runtime                 |
| GitHub variable | `RATE_AI_PER_SESSION_MINUTE`        | Cloud Run runtime                 |
| GitHub variable | `RATE_AI_PER_IP_MINUTE`             | Cloud Run runtime                 |
| GitHub variable | `DB_MAX_OPEN_CONNS`                 | Cloud Run runtime                 |
| GitHub variable | `DB_MAX_IDLE_CONNS`                 | Cloud Run runtime                 |
| GitHub variable | `CLOUD_RUN_MAX_INSTANCES`           | Cloud Run deploy flag             |
...
`deploy.yml` はこれらのGitHub Environment変数を認証・image buildより前に検証します。上表以外のruntime設定はvalidation済みのcode/image defaultを使います。productionへ進む前に、[`deployment.md`](deployment.md) のrelease gateで外部サービス側の未決値も確認してください。値が未決のため、この文書では新しいproduction値を確定していません。
