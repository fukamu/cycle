...
-> candidate-only old-image drainまたはexact image reuseをauthoritative metadataの連続2観測で確認
...
3. [`design.md` §41.5](design.md#415-csrf--session)のauthoritative drain条件を、Deploy前後のWorker deployment / version / tagとContainer application / rollout / instanceのbounded API取得で確認する。Deploy前baselineはmutation wakeより前に10秒間隔・最大5分で再取得し、active rollout IDがnull、instance inventoryがidleの0件または全件runningかつApplication version / imageと整合する同一semantic projectionを連続2回取得して初めて固定する。追加したpre-mutation待機で従来のmutation後budgetを削らないよう、Deploy job全体のtimeoutは50分とする。Active rollout IDは対応recordがcompletedでも待機理由とし、`placed`を含む非running state、version不一致、存在するimage reference不一致を受理しない。Pending stateの収束前やtimeout時はwakeせず、stable sample間でWorker / Application / rollout identity、versionまたはimageが変われば旧状態へ再束縛せず停止する。Rollout履歴は固定page上限の`limit` / `last` paginationで切り詰めを検出し、baseline以前から存在するhistorical rowのIDだけをnew-rollout相関の対象外として固定する。異なるimage digestへ進むcandidateでは、providerが後から更新またはinventoryから除外するpre-existing historical rowのstatus / version / image metadataをcandidateのactive stateやold-image drainの証拠に使わず、その変化だけでは停止しない。その件数にかかわらず、baseline後のnew rolloutはbaseline versionから観測中のcandidate version / imageへ遷移するexact candidate 1件だけであることを確認する。Candidate Worker version IDから固定Wranglerが生成した同一runner上のlocal image tagをshell非経由でinspectし、Wranglerがduplicate remote imageとしてlocal tagを除去して解決できない場合はbaseline Applicationのdigest-qualified Cloudflare Registry referenceをshell非経由のbounded manifest inspectで検証する。要求したexact reference、単一image media type、baselineと同じexact `sha256` digestだけを受理し、raw manifestはlog / artifactへ出さない。Digestがbaselineと異なる場合は、従来どおり一意なcompleted rollout、active rollout IDがnullのApplication、存在する全running instanceへ束縛する。Digestがbaselineと同じ場合は新しいContainer rolloutを捏造せず、digest解決後のsuccess候補となる各観測で新しいrolloutが存在しないこと、baseline rollout ID集合と各rowの全metadataがbaselineと一致すること、active rolloutがnull、instance inventoryがidleの0件であることを確認し、その同一状態を連続2回取得した場合だけ`containerProof=image_reused`として受理して`containerRolloutId=null`を記録する。同一digestで新しいrolloutが観測された場合は`unexpected_rollout`で即時停止し、同一imageとして受理しない。異digestは`containerProof=rolled_out`とし、非null rollout ID、異なるversion / digestを必須にする。連続観測ではinstance ID、配列順、または一時的な件数差だけでは失敗にしない。0件ではApplication / completed rolloutまたはexact reused digestにより今後のon-demand起動imageまで束縛し、後続candidate-public smokeで新しいruntime configurationを伴うprocessの実起動を必ず確認する。

   各観測ではinventory取得の前後でWorkerとApplicationのactive stateが変わらないことを別々に照合する。前後不一致の内容はcoherentなauthoritative observationではないため、その中間値からterminal stateやidentity矛盾を判定しない。Deploy前baselineの前後不一致は`worker_observation_changed`または`application_observation_changed`で即時停止し、再試行もwakeもしない。Deploy後drainではこの2reasonを既存deadlineと256観測上限内で再試行する。最初にcoherentなcandidate Workerを観測した時点でcandidate SHA tag、deployment ID、version IDとそのversionから解決したexact image digestを固定し、以後のcoherent observationはWorkerの3値が完全一致しなければならない。旧baselineへの回帰を含む不一致は`worker_identity_changed`で即時停止し、別candidateへ再束縛しない。

   Cross-endpoint visibility lagは、固定済みのexact candidate Worker、同じApplication ID、baselineと異なる解決済みexact candidate digest、矛盾のないinstance bindingのenvelope内だけでpendingとして扱う。Pre-existing historical rowはbaseline IDとの集合差でnew rolloutから除外するためだけに使い、そのmetadata driftやinventoryからの除外をvisibility lagの成立条件にしない。Applicationのcandidate version / imageまたはactive rollout IDが先に可視化され、対応する新rollout rowがまだinventoryにない場合は`rollout_inventory_visibility_lag`とする。Row欠落中にactive rollout IDがnon-nullならそのIDを直ちに固定し、以後は同じIDのrowが可視化された場合だけfull rollout identityへ昇格する。別IDのactive rolloutまたはrowへ再束縛しない。Baseline current versionから非baseline version / exact candidate imageへ進む、`reverted` / `replaced`ではないrollout rowが一意に先行し、競合する新rolloutがなく、active rolloutがnullまたはそのrowを指す一方でApplication version / imageがまだ完全なbaselineの場合は`application_rollout_visibility_lag`とする。既に固定したcandidate rolloutが一時的にinventoryから見えず、Applicationが同じcandidate version / imageを保つ場合も前者に含める。Visibility lag、candidate Application、full rollout identity固定後を含む全candidate stateで、baseline後のnew rolloutはexact candidate 1件だけを許可し、別version / imageのinactive rolloutを含む追加rowは`unexpected_rollout`で即時停止する。この2reasonも既存deadlineと256観測上限内だけで再試行する。同一観測内の前後不一致とvisibility lagはいずれも成功観測に数えず、既に得たcandidate stable sampleを破棄するため、その後に完全でcoherentなcandidate-only observationを改めて連続2回取得できるまで成功にしない。

   Coherent observation内のWorker / Application identity不明、同一digest reuse時のhistorical rollout drift、同一digestの新rollout、別digest、競合または変化したcandidate rollout、Application state矛盾、candidate image / instance binding矛盾、candidate rolloutの失敗terminalである`reverted` / `replaced`、矛盾するcandidate instanceは再試行対象にせず即時停止する。これらは`worker_identity_changed`、`application_identity_changed`、`rollout_history_changed`、`unexpected_rollout`、`application_state_changed`、`rollout_identity_changed`、`candidate_image_changed`、`instance_binding_changed`、`rollout_failed`、`candidate_stability_changed`の固定reasonへ分類する。Retry可能な4reasonを含め、reasonへprovider値、ID、version、image、stateまたはraw payloadを含めない。旧Worker version、deploy前Container image digest、candidate version / digest、proof種別、該当時だけのrollout、確認時刻、確認者だけをaccess-controlled drain-pending recordへ残し、API取得不能、candidate imageの一意なdigest取得不能、manifest schema不明、authoritativeなcandidate state driftまたはtimeoutでは停止する。
...
このone-time harnessの実行中は、初回harness導入前に`--tag`なしで配備されたbaselineを引き継ぐため、version annotation全体または`workers/tag`が存在しない状態を`null`として受理し、baselineのdeployment ID / version IDと連続観測の完全一致で旧版へ束縛する。存在するtagは常に40桁lowercase commit SHAとして検証する。Candidateは`wrangler deploy --tag "${COMMIT_SHA}"`、baselineから変わったdeployment ID / version ID、exact main SHAのtagをすべて満たさなければcandidateとして受理せず、未tagの新版からdrain証跡を生成しない。このnullable baseline表現は恒久的なdeploy contractではない。最初のtagged rollout、authoritative drain、candidate smokeを完了したら、one-time harnessの撤去と同時に削除し、以後のbaselineもexact SHA tagを必須に戻す。

Containers APIのschema移行では、deprecatedなApplication `instances` countの欠落を許容し、`max_instances: 1`と別endpointのinstance inventory取得は必須とする。Inventoryの0件はCloudflareのon-demand lifecycleによるidle stateとして受理し、1件以上では全instanceのstate / version / image bindingを必須とする。Application / rollout / instanceのdeployment versionは0から始まるnon-negative integerとして検証する。Placement stateは現行Wranglerと同じく`container_status`を優先し、欠落時だけ`health`へfallbackする。Instance `image`はAPI上optionalなため、欠落時は必須の`app_version`を介してApplication / candidate rolloutのversion-image bindingで判定し、値がある場合は同bindingとの一致を必須とする。Historical rolloutのoptionalなtarget imageは欠落を許容するが、candidate rolloutではApplication imageおよび上記のexact candidate digest evidenceと一致するtarget imageを必須とする。未知state、旧 / 新imageの矛盾、version不一致は受理しない。Fixed childのstdout / stderrはBrowser orchestratorへ渡さず、専用diagnostic descriptorはwrapperとdrain readerだけが保持し、その他のchild processでは閉じる。このdescriptorにrepository-owned processが生成するclosed enumの一行だけを渡す。Migration、current-main guard、secret materialization、Wrangler deploy等が失敗した場合は`Staging candidate deployment failed; source=...`、Baseline / drain readerが失敗した場合は`Cloudflare drain evidence failed; phase=...; reason=...; source=...`を転送する。Reader終了時はwrapperも対応する`baseline_handshake` / `drain_handshake`を出し、Cloudflare側の有効な一行とphaseが一致する許可済みpairだけはCloudflare側の詳細分類を優先する。Baselineのpending reasonは優先順に`baseline_active_rollout`、`baseline_instance_not_running`、`baseline_instance_version_mismatch`、`baseline_instance_image_reference_mismatch`とし、reason自体へrollout / instance ID、実state、version、imageまたはraw provider payloadを埋め込まない。`source=wrangler_deploy`はWorker activation、image build / push、Application更新、rollout開始を含むnon-transactionalなWrangler command全体の失敗を示し、どこまで適用済みかは確定しない。これらの`source`は固定分類だけを示し、path、response body、ID、credentialを含めない。有効な単独行または上記の許可済みpairへ一意に絞れない場合はraw detailを推測せず、generic `deploy_or_drain_failed`として扱う。
...
Candidate-public entryで初期Session discoveryが既存UIの`再試行`へ到達した場合、anonymous session POSTがまだrouteへ入っておらず、cleanup checkpoint失敗もなく、そのrunで未使用のときだけ、同じBrowser context、page、bootstrap ID、response captureを保持して`再試行`を1回だけ押します。Admissionまたは新規目標画面へ遷移しなければ停止し、2回目の自動再試行はしません。POST観測後、checkpoint失敗後、claim拒否時、遷移timeout時は`anonymous_session_request_not_observed`でfail closedとします。POSTが観測済みでcaptureが未確定なら、別の短いtimeoutを重ねずに同じcapture自身の既存上限まで確定を待ち、responseを分類できた場合は対応する`anonymous_session_bad_request`、`anonymous_session_forbidden`、`anonymous_session_rate_limited`、`anonymous_session_unavailable`または`unexpected_status`をentry timeoutより優先します。POST成功後にUI遷移だけが失敗した場合は、capture済みidentityをmemory内だけでfailure cleanupへ引き渡し、新しいanonymous accountを作成しません。POST未観測または観測callbackがない場合はcaptureを待たずにfail closedとします。
...
Backend Container imageは標準OCI metadataの`org.opencontainers.image.title`と`org.opencontainers.image.description`を持ちます。今回追加したdescriptionはtitleだけだった直前imageとの差分として一度だけ新しいdigestを作ります。以後はlabel値を固定し、将来のharness-only recoveryでfresh digestを作る仕組みとして再利用または書換えしません。同一digestのforward recoveryは上記の`image_reused` proofで扱い、metadataだけを変えた不要なContainer rolloutを作りません。このmetadata差分をApplication修正、candidate-only state、post-deploy smokeの代替証拠にはしません。
