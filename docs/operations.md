...
Containers APIのschema移行では、deprecatedなApplication `instances` countの欠落を許容する一方、`max_instances: 1`と別endpointから取得する非emptyなinstance一覧は引き続き必須とする。Placement stateは現行Wranglerと同じく`container_status`を優先し、欠落時だけ`health`へfallbackする。未知state、旧 / 新image混在、version不一致は受理しない。Baseline / drain readerが失敗した場合、Browser orchestratorはproviderのraw responseやstderrを表示せず、検証済みclosed enumの`Cloudflare drain evidence failed; phase=...; reason=...`を一行だけ転送する。この行がない場合はraw detailを推測せず、generic `deploy_or_drain_failed`として扱う。
