...
今回のdiagnostic forward fixはruntime挙動を変えないOCI image labelを追加し、直前の部分deployでContainerだけが既に更新済みの場合も新しいcandidate image digestを生成してfresh rolloutを証明可能にする。この一回限りのlabel変更をApplication修正やdrain証跡の代用にしない。
