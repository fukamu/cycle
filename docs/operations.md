...
3. [`design.md` §41.5](design.md#415-csrf--session)のauthoritative drain条件を、Deploy前後のWorker deployment / version / tagとContainer application / rollout / instanceのbounded API取得で確認する。Deploy前baselineはmutation wakeより前に10秒間隔・最大5分で再取得し、active rollout IDがnull、instance inventoryがidleの0件または全件runningかつApplication version / imageと整合する同一semantic projectionを連続2回取得して初めて固定する。追加したpre-mutation待機で従来のmutation後budgetを削らないよう、Deploy job全体のtimeoutは50分とする。Active rollout IDは対応recordがcompletedでも待機理由とし、`placed`を含む非running state、version不一致、存在するimage reference不一致を受理しない。Pending stateの収束前やtimeout時はwakeせず、stable sample間でWorker / Application / rollout identity、versionまたはimageが変われば旧状態へ再束縛せず停止する。Rollout履歴は固定page上限の`limit` / `last` paginationで切り詰めを検出し、baseline後に増えた履歴全体の件数ではなく、baseline versionから観測中のcandidate version / imageへ遷移するrolloutを一意に相関する。Candidate Worker version IDから固定Wranglerが生成した同一runner上のlocal image tagをshell非経由でinspectし、Wranglerがduplicate remote imageとしてlocal tagを除去して解決できない場合はbaseline Applicationのdigest-qualified Cloudflare Registry referenceをshell非経由のbounded manifest inspectで検証する。要求したexact reference、単一image media type、baselineと同じexact `sha256` digestだけを受理し、raw manifestはlog / artifactへ出さない。Digestがbaselineと異なる場合は、従来どおり一意なcompleted rollout、Application、存在する全running instanceへ束縛する。Digestがbaselineと同じ場合は新しいContainer rolloutを捏造せず、Application version / imageとrollout全metadataがbaselineから不変、active rolloutがnull、instance inventoryがidleの0件である状態を連続2回取得した場合だけ`containerProof=image_reused`として受理し、`containerRolloutId=null`を記録する。異digestは`containerProof=rolled_out`とし、非null rollout ID、異なるversion / digestを必須にする。連続観測ではinstance ID、配列順、または一時的な件数差だけでは失敗にしない。0件ではApplication / completed rolloutまたはexact reused digestにより今後のon-demand起動imageまで束縛し、後続candidate-public smokeで新しいruntime configurationを伴うprocessの実起動を必ず確認する。各観測の前後でactive stateが変わらないことも照合する。旧Worker version、deploy前Container image digest、candidate version / digest、proof種別、該当時だけのrollout、確認時刻、確認者だけをaccess-controlled drain-pending recordへ残し、API取得不能、candidate imageの一意なdigest取得不能、manifest schema不明、state driftまたはtimeoutでは停止する。
