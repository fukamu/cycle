...
- stable v1初回rolloutは§27.2のdual-validationを含む。Cloudflare deploy成功は旧imageのdrain完了を意味せず、mixed-version中は旧Backendが返したlegacy tokenが一度`CSRF_INVALID`となり得るavailability上のdegraded behaviorを受容する。Origin、CSRF、Expected User、Session guardを緩和しない。Deploy前のWorker deployment / versionとContainer Application imageのbaselineは、release mutationをwakeする前にbounded timeout内でpollし、同一のstable semantic projectionを連続2回取得して固定する。Active rollout IDが存在する状態は対応rollout recordがcompletedでもpendingとし、非空instanceの`placed` / `stopping` / `stopped` / `failed` / `unhealthy`、Applicationとのversion不一致または存在するimage reference不一致も受理せず再観測する。収束しなければ最後のclosed pending reasonまたは`evidence_timeout`で停止してwakeせず、2件のstable sample間でWorker / Application / rollout identity、versionまたはimageが変われば`evidence_changed`として旧baselineへ黙って再束縛しない。Deploy後はcandidate SHAにtag付けされたWorker versionだけが100% trafficを受け、candidateへのContainer rolloutがcompletedで、Application imageがexact candidate digestと一致し、instance inventoryがidleの0件または全件running candidateであるauthoritative Cloudflare metadataを連続2回取得できた場合だけdrain完了とする。ただし固定Wranglerが同一imageを検出してlocal tagを除去しContainer rolloutが発生しない場合は、baseline Applicationのdigest-qualified registry referenceが同じexact digestの単一image manifestを指し、Application version / imageとrollout全metadataがbaselineから不変、active rolloutがnull、instance inventoryがidleの0件である状態を連続2回取得した場合だけ`image_reused`として受理する。0件は旧processが存在せず、次回on-demand起動がcandidate Application imageと新しいruntime configurationに束縛されるidle stateとしてのみ受理し、後続candidate-public smokeで実際のBackend到達とstable挙動を必ず確認する。Inactive / unknownな非空state、旧image残存、API schema不明、取得不能、timeoutは成功扱いにせず、drain確認後だけmulti-tab smokeを行う。
...
6. Authoritative Cloudflare metadataからold-image drainをbounded pollし、candidate-only stateを連続2回確認する。固定Wranglerが同一imageを検出してlocal tagを除去しContainer rolloutが発生しない場合は、baseline Applicationのdigest-qualified registry referenceが同じexact digestの単一image manifestを指し、Application version / imageとrollout全metadataが不変、active rolloutがなく、instance inventoryがidleの0件である状態を連続2回確認した場合だけ`image_reused`として受理する。後続candidate-public requestで新しいruntime configurationを伴うprocessをon-demand起動し、証跡不能、state変化またはtimeoutでは停止する
