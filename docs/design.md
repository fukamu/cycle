...
- stable v1初回rolloutは§27.2のdual-validationを含む。Cloudflare deploy成功は旧imageのdrain完了を意味せず、mixed-version中は旧Backendが返したlegacy tokenが一度`CSRF_INVALID`となり得るavailability上のdegraded behaviorを受容する。Origin、CSRF、Expected User、Session guardを緩和しない。Release mutation前に現在のWorker / Containerのstable baselineを固定し、固定不能ならdeployをwakeしない。Deploy後はcandidate SHAにtag付けされたWorker versionだけが100% trafficを受け、candidateへのContainer rolloutがcompletedかつactive rollout IDがnullで、Application imageがexact candidate digestと一致し、instance inventoryがidleの0件または全件running candidateであるcoherentなauthoritative metadataを連続2回取得できた場合だけdrain完了とする。Boundedなprovider visibility lagは成功観測に数えず、既存deadline内で再観測した後も完全なcandidate-only stateを改めて連続2回要求する。Coherent observation内のcandidate identity、new rollout、image、instanceまたはcandidate rolloutのterminal stateの矛盾は即時停止し、candidate identityを別状態へ再束縛しない。異なるimage digestへのcandidate rolloutでは、baseline以前のhistorical rollout rowはnew-rollout相関用のID集合としてだけ扱い、その後のmetadata更新またはinventory除外をcurrent candidateやold-image残存の証拠として扱わない。Baseline、coherence、lag envelope、closed reasonとretryのexact procedureは[`operations.md`](operations.md#session-bound-stable-csrf-v1-release)が所有する。ただし固定Wranglerが同一imageを検出してlocal tagを除去しContainer rolloutが発生しない場合は、baseline Applicationのdigest-qualified registry referenceが同じexact digestの単一image manifestを指し、digest解決後のsuccess候補となる各観測で新しいContainer rolloutが存在せず、Application version / imageとrollout全metadataがbaselineと一致し、active rolloutがnull、instance inventoryがidleの0件である状態を連続2回取得した場合だけ`image_reused`として受理する。同一digestで想定外のrolloutが存在する状態は受理しない。0件は旧processが存在せず、次回on-demand起動がcandidate Application imageと新しいruntime configurationに束縛されるidle stateとしてのみ受理し、後続candidate-public smokeで実際のBackend到達とstable挙動を必ず確認する。Inactive / unknownな非空state、旧image残存、API schema不明、取得不能、timeoutは成功扱いにせず、drain確認後だけmulti-tab smokeを行う。
...
- Goal Deleteのserver `204`またはownerを秘匿した厳密な`GOAL_NOT_FOUND`確認後、senderはdurable cleanupより先に`version / deletedUserId / deletedGoalId`だけを持つversioned same-origin advisoryをpublishする。厳密な`GOAL_NOT_FOUND`はCycle Workspace / Goal Reviewの初期読込、autosave、競合回復、AI command、状態遷移に加え、Goal Timelineの初期Goal読込とCycle一覧のinitial / refetch / paginationのどのresponseで確認した場合も同じdeletion commitへ収束させ、単なる`404`、別error code、`409`、network errorは根拠にしない。該当User / Goalを表示する受信tabはcallback内でeditorとleaseを同期的にfenceし、autosaveをquiesceしてから同じdurable cleanupを行う。別User / Goalは無視し、受信通知を再publishしない。senderはcleanup成功後に同じ通知を一度再送でき、受信側は重複をcoalesceする。BroadcastChannelはbest-effortであり、suspend / offline / delivery失敗時もdurable tombstoneを正本とする。
...
6. Authoritative Cloudflare metadataからold-image drainをbounded pollし、coherentなcandidate-only stateを連続2回確認する。Boundedなprovider visibility lagは成功観測に数えず、完全な連続確認をやり直す。固定Wranglerが同一imageを検出してlocal tagを除去しContainer rolloutが発生しない場合は、baseline Applicationのdigest-qualified registry referenceが同じexact digestの単一image manifestを指し、新しいContainer rolloutがなく、Application version / imageとrollout全metadataが不変、active rolloutがなく、instance inventoryがidleの0件である状態を連続2回確認した場合だけ`image_reused`として受理する。Exactな判定手順は[`operations.md`](operations.md#session-bound-stable-csrf-v1-release)に従い、後続candidate-public requestで新しいruntime configurationを伴うprocessをon-demand起動する。証跡不能、state変化またはtimeoutでは停止する
...
Generic pre-switch hard gateはcurrent-publicのhealth / readinessとmigration precondition / revision evidenceに限定する。Current-publicのAdmission entry、Turnstile anonymous bootstrap、session discovery、公開account delete、削除後session 401は`baseline` modeのself-cleaning diagnosticとして別のoperator調査runでだけ実行でき、その失敗をcandidate deployの成否根拠にしない。Stable CSRF初回rolloutではlegacy compatibilityをexact-main CIの決定的な証跡で確認し、live anonymous Sessionはauthoritative drain後のcandidate-publicだけで一度作成する。同一Deploy runでgeneric `baseline`を実行せず、current-publicのTurnstileやanonymous-create rate-limitを消費しない。Candidate-publicのstable two-tab / security smoke、health / readiness、critical journeyまたはcleanup proofが失敗した場合はreleaseを成功とせず、Recovery modeやruntime inputでskipしない。Migration失敗時はApplication deployを行わない。
...
`Deploy Staging`のworkflow rerunは、同じworkflow runのattempt 1が`completed` / `failure`であり、そのattempt 1が自動生成したexact checkpointを検証できる場合に限り、`Re-run all jobs`によるattempt 2を一度だけ許可する。Checkpointはfixed child内のmigration command呼び出し直前をrelease mutation boundaryとし、`not_crossed/not_started`から`crossed/not_started`へ進める。Authoritative drain後のtemporary account create request直前に`crossed/unverified`、公開Delete 204 / 旧Session 401の確認後に`crossed/verified`へ進め、逆遷移を許可しない。`mutationBoundary=not_crossed`かつ`cleanupState=not_started`の場合だけ`no_mutation_started`を表明する。同じfileをhuman audit用artifactと、rerun間のruntime transportであるexact immutable Actions cache key `staging-deploy-retry-<commit>-<run-id>-1`へ保存し、attempt 2はfallback keyを使わずexact cache hitとstrict file validationを必須とする。Checkpointはrepository、workflow、run、source attempt 1、candidate SHA、mode、operator、exact-main CI、mode固有のTerraform evidenceへ束縛し、attempt 2はcurrent main、成功CI、通常modeのTerraform evidenceまたはrecovery modeの空のTerraform evidenceを新たに解決・再検証する。Attempt 2と同じattemptで生成されたfresh resolve outputを必須とし、deploy jobだけを対象にして成功済みresolveを再実行しないfailed / selected-job rerunを拒否する。Attempt 3以降、cache miss、checkpoint fileの欠落・破損・binding不一致、cleanup `unverified` / `verified`、release mutation boundary以降、cancel / timeoutまたは状態不明でもrerunを拒否し、partial resumeしない。
...
Stable CSRFでは、固定key / Session IDのbyte-level golden vectorによりscope文字列、NUL separator、lowercase UUID、paddingなしbase64urlとverifier式を固定する。同じSession / keyの同値性、Sessionまたはkey変更時の差、空・不正token拒否、legacy verifier収束、同一hashのidempotent保存、revoke / expiry raceをunit / repository testで検証する。Application unit testはstable側が一致する場合もlegacy verifier側のconstant-time比較を実行してからORすることをcomparator call countで固定する。HTTP integrationはbarrier同期した2件以上の`GET /session`が同じtokenを返し、各tokenによるunsafe Requestがともに成功すること、invalid token / Origin、revoked / expired /旧Sessionを拒否すること、Session rotation後は旧CSRFを拒否して新CSRFが成功することを検証する。Pepper rotation testはmaintenanceとold-instance drain後の単一active keyだけを対象とし、旧token拒否と新token成功を固定する。Rolloutのdeterministic testはlegacy / stable token、旧 / 新Application、旧 / 新key、expiry / revoke raceをexact-main CI evidenceで固定する。Live Stagingはcandidate-only authoritative drain evidenceの後にfresh anonymous Sessionを作成し、同一Browser Context二tabの同時Session discoveryの同値、片tab reload後の両tab unsafe command / autosave、invalid token / Origin拒否、Account Delete後の401、advisory欠落時のauthoritative recoveryを検証する。Legacy verifier収束とconvergence後のlegacy token拒否は同じexact-main CI evidenceが所有し、current-publicのTurnstile / legacy Session取得をlive release gateへ重複させない。TTL短縮、DB直接操作、test-only Production endpoint、pepper変更でlive caseを作らない。
...
Staging release fixtureは、current-publicのblocking preflightがhealth / readinessだけを呼び、Stable CSRF初回rollout中はgeneric anonymous diagnosticを同じDeploy runで重複実行せず、exact-main CIのlegacy互換性証跡を確認してからrelease mutationへ進むことを固定する。#139のlive Session作成、stable two-tab / security smokeとcleanupはauthoritative drain後のcandidate-public hard gateとする。Manual `baseline` failureは`target=current-public` / `mutation_started=false`のwarning、candidate `full` failureは`target=candidate-public` / `mutation_started=true`のerrorとし、全modeのcleanup state遷移とsecret-safe field allowlistをunit / workflow contractで検証する。
