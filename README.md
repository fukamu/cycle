...
`http://localhost:5173` を開きます。同名のPostgreSQL containerを以前作成済みなら、imageが正確に`postgres:18.6-alpine3.24@sha256:d3e1620b530c944afa6e887d22eb899824da68e19c52024bf98f5220c88a65b2`であることを確認した場合だけ、`docker run`の代わりに`docker start fukamu-cycle-postgres`を使います。異なるimageは再利用しません。既存 `.env` / `.env.local` はsetup scriptで上書きされません。
