...
test("deployment contract is the exact repository handoff classification", () => {
  ...
  for (const forbiddenLiveJourneyInput of [
    "check-staging-critical",
    "STAGING_BASE_URL",
    "STAGING_E2E_INVITE_TOKEN",
    "playwright install",
  ]) {
    assert.equal(
      deployJob.includes(forbiddenLiveJourneyInput),
      false,
      `deployment must not retain live Browser journey input: ${forbiddenLiveJourneyInput}`,
    );
  }
  ...
  assert.ok(
    ...
    "frontend build, runtime validation, migration, traffic switch, and smoke order",
  );
