...
- 初版作成日: 2026-08-22
- 状態: IN_PROGRESS / M21 sqlc Cycle/AI/schema cleanup
...
- 現在の再開位置: M21 sqlc Cycle/AI/schema cleanup。Cycle read/write/receipt、AI Generation/Usage/rate/budgetの残るstable raw SQLをtransaction-bound generated Queriesへ移し、unused generated surfaceを整理する。M18で確定したcanonical provider input hashとidempotency request hashの分離、AI Usage settlement metadata・15分retention・migration-first fail-closed Account Delete guardは既存migrationを編集せず新規`000002+` expand/backfill/constraintとして実装し、empty DB up、disposable DB down/upを検証する。
