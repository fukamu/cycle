...
- 現在の再開位置: M19 OpenAI transport contract。Live AIは使用せず、`httptest`でResponses APIのmodel、strict schema、`store=false`、status、timeout/cancel、usage/cost mappingを固定する。M18のplan-inclusive staged Cと独立commit成功後、transport contractのREDから着手する。
