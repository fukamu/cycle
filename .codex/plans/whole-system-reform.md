...
- 初版作成日: 2026-08-22
- 状態: IN_PROGRESS / M22 OTel SDK/exporter composition
...
- 現在の再開位置: M22 OTel SDK/exporter composition。Local/testのin-memory exporterとStaging/ProductionのOTLP/HTTP exporterを既存vendor-neutral configへ接続し、実span/metric送信、resource attribute、shutdown flushをBackend integrationで固定する。Staging deployやendpoint secret変更は行わず、M22 acceptanceどおりB、in-memory OTLP receiver integration、A、Cを実行する。
