...
- 初版作成日: 2026-08-22
- 状態: IN_PROGRESS / M18 Typed AI ports
...
- 2026-08-24: M14監査で、現schemaの`ai_generations.input_hash`がidempotency request identityを保存しており、SoT §37.7のcanonical provider input（model/prompt/context/data）hashと意味が衝突していることを確認した。Baseline migrationは編集せず、M18でtyped canonical provider input/hash builderを導入し、M21で新規`000002+` expand migrationとして`idempotency_request_hash`を追加・既存値をbackfillした後、`input_hash`をcanonical provider input専用へ切り替える。M14では既存idempotency/wire/storage挙動を維持する。
...
- 現在の再開位置: M18 Typed AI ports。Generic `AIProvider.Execute`とoperation stringをGoal Refine / Action Generate / Action Refineのtyped portsへ分割し、Fake/OpenAIが返すraw typed resultを共通Domain validatorへ通す。Goal suggestionはnewline正規化・nonblank/NUL/control拒否・80 code points、Action Generateは1–3 raw itemsを各正規化して`1. ...`形式へcanonical renderし全体200 code points、Action Refineはnonblank/NUL/control拒否・200 code pointsをApplicationで一意に保証する。Token/output entitlement、prompt/context selection、共通retry validation、canonical provider input/hash builderをApplicationへ移す。同時に残るAction AI Begin/Finish orchestrationをtyped Application Unit of Workへ移し、User→Goal→Cycle→Generation→Budget lock、lease recovery、quota/rate/budget、provider外部call、exact finalizationを所有させ、Postgresのhigh-level Action orchestrationとfloat64 budget controlを除去する。現`ai_generations.input_hash`はidempotency request identityなのでM18ではstorage意味を変更せず、canonical hashの永続化と`idempotency_request_hash` migrationはM21へ留保する。公開wireは不変、M19はstrict OpenAI transport/schema/status/timeout/cancel/usage/costだけを所有する。M17のplan-inclusive staged Cと独立commit成功後、Domain/Application/adapterのREDから着手する。
