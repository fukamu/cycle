...
- 初版作成日: 2026-08-22
- 状態: IN_PROGRESS / M17 Review transition use cases
...
- 現在の再開位置: M17 Review transition use cases。ContinueReview / Terminateを単一`ReviewTransitionUseCases` boundaryへ移し、ApplicationがDomain transition、canonical hash、ID/clock、receipt double-check、replay materialization、Retention partition、lock順、exact affected-row invariantを所有する。ContinueはGoal lock待機後にreceiptを再確認し、同一User・別Goalの同じoperation IDをraw unique errorにせずtyped Cycle claimとreceipt再読で分類する。TerminateはUser→Goal→CycleまたはReview Draft/Generation/Usageの順でlockし、`activeCycleId`とdiscard confirmationをhashへ含め、HTTP requestをstate別discriminated unionとして400で検証し、active replayのcanceled Cycleを復元する。Review terminalは24時間15分境界でfinalized Usageだけを物理削除し、未確定/期間内Usageは本文redactで保持する。Continueのcurrent Version不整合は既存`GOAL_VERSION_CONFLICT`、callerがVersion preconditionを持たないTerminateの不整合は新しい409を推測せずpersistence invariantとしてfail-closedにする。GetReview read path、route/response shape、明示READ COMMITTED、Complete/Continue/Terminateのglobal lock順を維持し、double-tap、response-loss replay、Continue/Terminate両winner、retention exact boundary、cross-user 404をApplication/unit・actual HTTP・deterministic実DBtestで固定する。M16のplan-inclusive staged Cと独立commitが成功した直後から着手する。
