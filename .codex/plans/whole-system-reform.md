...
- 初版作成日: 2026-08-22
- 状態: IN_PROGRESS / M29 React Hook Form removal dependency slice staged final C
...
- 現在の再開位置: M29のReact Hook Form削除独立slice。Controlled React state、manifest/lockfile、SoT、zero-reference、frozen install、target/F、actual S、plan-inclusive A、fresh disposable `*_test` PostgreSQLによるEはGREEN。全変更をstageして別fresh DBでplan-inclusive final Cを完走後、index/working treeを変えず独立commitする。Production/Staging deploy/apply、secret変更、live data操作は禁止したまま維持する。
