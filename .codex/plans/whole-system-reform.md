...
- 初版作成日: 2026-08-22
- 状態: IN_PROGRESS / M16 Cycle edit/complete use cases
...
- 現在の再開位置: M16 Cycle edit/complete use cases。ListCycles / GetCycle / SaveFrame / CompleteCycleを単一`CycleUseCases` boundaryへ移し、Applicationがcursor/page、Domain transition、stale same-body no-op、double receipt replay、request hash、ID/clock、User→Goal→Cycle lock順、exact affected-row invariantを所有する。Postgresはdecoded keyset queryとtyped Cycle Unit of Work/Tx primitiveだけを提供する。Completeの入力不足はplan/do/check/action順の`missingFrames`を持つ`CYCLE_COMPLETION_INPUT_INCOMPLETE`、stale expected Goal revisionは`GOAL_VERSION_CONFLICT`へSoTどおり修正し、現行route/wire、SaveFrameの別frame独立性、Complete `READ COMMITTED`、current GoalVersion整合性をcharacterizationと実DBtestで固定する。M15のplan-inclusive staged Cと独立commitが成功した直後から着手する。
