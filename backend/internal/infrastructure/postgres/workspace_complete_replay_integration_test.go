...
func isGoalViewLookupQuery(sql string) bool {
	...
	return strings.Contains(normalized, "from goals g join goal_versions gv") &&
...
content_revision=4,plan_revision=1,do_revision=1,check_revision=1,action_revision=1 WHERE id=$1`, fixture.cycleID); err != nil {
	...
	completed, err := seedStore.CompleteCycle(context.Background(), completeInput)
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		result, callErr := store.CompleteCycle(replayCtx, completeInput)
