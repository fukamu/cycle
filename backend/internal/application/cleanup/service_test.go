...
func TestDryRunUsesOneUTCReadOnlyRepositorySnapshot(t *testing.T) {
	repository := &fakeRepository{countResult: CandidateCounts{AIUsageEvents: 3, AbuseRateBuckets: 5}}
	...
	if repository.aiCalls != 0 || repository.abuseCalls != 0 {
		t.Fatalf("dry-run mutation calls = ai:%d abuse:%d", repository.aiCalls, repository.abuseCalls)
	}
	if result.Mode != ModeDryRun || result.AIUsageEvents.CandidateCount != 3 || result.AbuseRateBuckets.CandidateCount != 5 ||
...
func TestExecuteUsesFixedDeadlineAndBoundedResourceSpecificBatches(t *testing.T) {
	repository := &fakeRepository{aiResults: []int64{2, 1, 0}, abuseResults: []int64{2, 2, 0}}
	service := NewService(repository)
	...
	if repository.aiCalls != 3 || repository.abuseCalls != 3 {
		t.Fatalf("batch calls = ai:%d abuse:%d, want 3/3 including terminal empty transactions", repository.aiCalls, repository.abuseCalls)
	}
...
func TestCleanupRejectsZeroDeadlineWithoutRepositoryAccess(t *testing.T) {
	...
	if repository.countCalls != 0 || repository.aiCalls != 0 || repository.abuseCalls != 0 {
...
func TestExecuteRejectsInvalidBatchWithoutRepositoryMutation(t *testing.T) {
	for _, batchSize := range []int64{-1, 0, MaxBatchSize + 1} {
		t.Run(fmt.Sprintf("%d", batchSize), func(t *testing.T) {
			...
			if repository.aiCalls != 0 || repository.abuseCalls != 0 || repository.countCalls != 0 {
...
func TestExecuteFailsClosedOnImpossibleRepositoryCount(t *testing.T) {
	...
	if !errors.Is(err, ErrRepositoryState) || repository.abuseCalls != 0 {
		t.Fatalf("invalid repository count error/calls = %v/%d", err, repository.abuseCalls)
	}
}
...
func TestCanceledCleanupDoesNotStartAnotherTransaction(t *testing.T) {
	...
	if !errors.Is(err, context.Canceled) || repository.aiCalls != 0 || repository.abuseCalls != 0 {
		t.Fatalf("canceled execute = %v, calls ai:%d abuse:%d", err, repository.aiCalls, repository.abuseCalls)
	}
}
