...
function validatePullRequestReviewTemplate() {
  ...
  if (template === undefined) {
    addProblem(
      ...
      ".github/pull_request_template.md must provide the Source of Truth review prompts",
    );
    return;
  }

  // This guard verifies only that the human-review prompts remain visible. It
  // intentionally does not prescribe their full wording or validate completed
  // pull request bodies.
  const visibleTexts = template.visibleTextBlocks.map((block) => block.text);
  const hasVisibleText = (pattern) =>
    visibleTexts.some((text) => pattern.test(text));
  const hasHeading = (level, pattern) =>
    template.headings.some(
      (heading) => heading.level === level && pattern.test(heading.text),
    );

  if (!hasVisibleText(/\bIssue\s*\/\s*Decision\s+URL\b/i)) {
    addProblem(
      templatePath,
      1,
      "PULL_REQUEST_TEMPLATE_DECISION",
      "Pull request template must retain a visible Issue / Decision URL prompt",
    );
  }
  ...
  const classificationPresent =
    hasHeading(3, /\bSpecification\s+Impact\s+classification\b/i) &&
    classifications.every((classification) =>
      visibleTexts.some(
        (text) =>
          /^\[[ xX]\]\s*/.test(text) && text.includes(classification),
      ),
    );
  if (!classificationPresent) {
    addProblem(
      ...
      "Pull request template must retain the three visible Specification Impact choices",
    );
  }

  if (!hasVisibleText(/\bCanonical\b.*\b(?:design\s+)?sections?\b/i)) {
    addProblem(
      templatePath,
      1,
      "PULL_REQUEST_TEMPLATE_CANONICAL_REFERENCE",
      "Pull request template must retain a visible canonical design section prompt",
    );
  }

  if (
    !hasVisibleText(
      /\bAffected\b.*(?:\bowners?\b.*\bareas?\b|\bareas?\b.*\bowners?\b)/i,
    )
  ) {
    addProblem(
      templatePath,
      ...
      "Pull request template must retain a visible affected owners / areas prompt",
    );
  }

  if (!hasVisibleText(/\bClassification\s+rationale\b/i)) {
    addProblem(
      templatePath,
      1,
      "PULL_REQUEST_TEMPLATE_RATIONALE",
      "Pull request template must retain a visible classification rationale prompt",
    );
  }

  const approvalPresent = visibleTexts.some(
    (text) =>
      /\bProduct Owner\b.*\bapproval\b.*\bURL\b/i.test(text) &&
      text.includes("仕様変更") &&
      text.includes("のみ"),
  );
  if (!approvalPresent) {
    addProblem(
      ...
      "Pull request template must retain a visible specification-change-only Product Owner approval URL prompt",
    );
  }

  const verificationPresent =
    hasHeading(2, /\bVerification\s+evidence\b/i) &&
    hasVisibleText(/\bCommands?\b.*\b(?:results?|evidence)\b/i);
  if (!verificationPresent) {
    addProblem(
      templatePath,
      1,
      "PULL_REQUEST_TEMPLATE_VERIFICATION_EVIDENCE",
      "Pull request template must retain visible verification evidence prompts",
    );
  }
}
