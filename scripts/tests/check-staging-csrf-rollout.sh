#!/usr/bin/env bash

set -Eeuo pipefail
IFS=$'\n\t'

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
repo_root="$(realpath -e -- "${script_dir}/../..")"
test_root="$(mktemp -d)"
trap 'rm -rf -- "${test_root}"' EXIT

fail() {
  printf 'not ok - %s\n' "$*" >&2
  exit 1
}

assert_failure() {
  local description="$1"
  shift
  if "$@" >"${test_root}/last-output" 2>&1; then
    fail "${description} unexpectedly succeeded"
  fi
}

bin="${test_root}/bin"
log="${test_root}/command.log"
output="${test_root}/output.log"
mkdir -p -- "${bin}"
cat >"${bin}/node" <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
[[ "$#" == "1" ]]
[[ "$1" == "${EXPECTED_HARNESS_PATH}" ]]
[[ "${STAGING_BASE_URL}" == "https://cycle.staging.fukamu.matoruru.com" ]]
[[ "${STAGING_ADMISSION_MODE}" == "auto" || "${STAGING_ADMISSION_MODE}" == "off" || "${STAGING_ADMISSION_MODE}" == "closed" ]]
if [[ "${STAGING_ADMISSION_MODE}" != "off" ]]; then
  [[ "${STAGING_E2E_INVITE_TOKEN}" =~ ^fukamu_cycle_beta_[A-Za-z0-9_-]{43}$ ]]
else
  [[ -z "${STAGING_E2E_INVITE_TOKEN+x}" ]]
fi
[[ -z "${DEBUG+x}" ]]
[[ -z "${NODE_DEBUG+x}" ]]
[[ -z "${NODE_OPTIONS+x}" ]]
[[ -z "${PWDEBUG+x}" ]]
printf 'node %s\n' "$1" >"${TEST_COMMAND_LOG}"
printf 'admission=%s\n' "${STAGING_ADMISSION_MODE}" >>"${TEST_COMMAND_LOG}"
EOF
chmod +x -- "${bin}/node"

token="fukamu_cycle_beta_$(printf 'A%.0s' {1..43})"
env \
  PATH="${bin}:${PATH}" \
  TEST_COMMAND_LOG="${log}" \
  EXPECTED_HARNESS_PATH="${repo_root}/frontend/e2e/staging-csrf-rollout.mjs" \
  DEBUG="pw:api" \
  NODE_DEBUG="module" \
  NODE_OPTIONS="--trace-warnings" \
  PWDEBUG="console" \
  STAGING_BASE_URL="https://cycle.staging.fukamu.matoruru.com" \
  STAGING_ADMISSION_MODE="closed" \
  STAGING_E2E_INVITE_TOKEN="${token}" \
  bash "${repo_root}/scripts/check-staging-csrf-rollout.sh" >"${output}" 2>&1

expected="$(
  printf '%s\n' \
    "node ${repo_root}/frontend/e2e/staging-csrf-rollout.mjs" \
    "admission=closed"
)"
[[ "$(cat "${log}")" == "${expected}" ]] || fail "rollout wrapper did not invoke the exact harness"
if grep -Fq -- "${token}" "${output}" || grep -Fq -- "${token}" "${log}"; then
  fail "rollout wrapper exposed the invite token"
fi

env \
  PATH="${bin}:${PATH}" \
  TEST_COMMAND_LOG="${log}" \
  EXPECTED_HARNESS_PATH="${repo_root}/frontend/e2e/staging-csrf-rollout.mjs" \
  STAGING_BASE_URL="https://cycle.staging.fukamu.matoruru.com" \
  STAGING_ADMISSION_MODE="off" \
  bash "${repo_root}/scripts/check-staging-csrf-rollout.sh" >"${output}" 2>&1
[[ "$(tail -n 1 "${log}")" == "admission=off" ]] || fail "off-mode rollout wrapper failed"

assert_failure "rollout wrapper with arguments" \
  env STAGING_BASE_URL="https://cycle.staging.fukamu.matoruru.com" \
  STAGING_ADMISSION_MODE="off" \
  bash "${repo_root}/scripts/check-staging-csrf-rollout.sh" unexpected
assert_failure "rollout wrapper without base URL" \
  env -u STAGING_BASE_URL STAGING_ADMISSION_MODE="off" \
  bash "${repo_root}/scripts/check-staging-csrf-rollout.sh"
assert_failure "rollout wrapper with blank base URL" \
  env STAGING_BASE_URL="   " STAGING_ADMISSION_MODE="off" \
  bash "${repo_root}/scripts/check-staging-csrf-rollout.sh"
assert_failure "rollout wrapper without admission mode" \
  env -u STAGING_ADMISSION_MODE \
  STAGING_BASE_URL="https://cycle.staging.fukamu.matoruru.com" \
  bash "${repo_root}/scripts/check-staging-csrf-rollout.sh"
assert_failure "rollout wrapper with invalid admission mode" \
  env STAGING_BASE_URL="https://cycle.staging.fukamu.matoruru.com" \
  STAGING_ADMISSION_MODE="private" \
  bash "${repo_root}/scripts/check-staging-csrf-rollout.sh"
assert_failure "rollout wrapper without invite token" \
  env -u STAGING_E2E_INVITE_TOKEN \
  STAGING_BASE_URL="https://cycle.staging.fukamu.matoruru.com" \
  STAGING_ADMISSION_MODE="auto" \
  bash "${repo_root}/scripts/check-staging-csrf-rollout.sh"
assert_failure "rollout wrapper with blank invite token" \
  env STAGING_BASE_URL="https://cycle.staging.fukamu.matoruru.com" \
  STAGING_ADMISSION_MODE="closed" \
  STAGING_E2E_INVITE_TOKEN=$' \t ' \
  bash "${repo_root}/scripts/check-staging-csrf-rollout.sh"

printf '%s\n' "Staging CSRF rollout wrapper tests passed."
