...
fail_configuration() {
  ...
  printf '%s\n' "::error::Staging critical failed; phase=configuration; reason=unexpected_status; run_id=${run_id}; run_attempt=${run_attempt}; commit_sha=${commit_sha}." >&2
  exit 1
}
...
if [[ "${STAGING_CRITICAL_MODE:-}" != "baseline" && "${STAGING_CRITICAL_MODE:-}" != "full" ]]; then
  fail_configuration
fi
